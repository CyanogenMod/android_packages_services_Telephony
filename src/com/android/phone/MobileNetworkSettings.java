...
public class MobileNetworkSettings extends PreferenceActivity
	...
	boolean isLteCapable = getResources().getBoolean(R.bool.config_enabled_lte);
        final int phoneSubId = mPhone.getSubId();
        ...
        if (carrierConfig.getBoolean(CarrierConfigManager.KEY_HIDE_CARRIER_NETWORK_SETTINGS_BOOL)) {
            ...
            if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                ...
                //check if phone has system property to control lte option
                SystemProperties.getBoolean("ro.telephony.lte_capable",isLteCapable);
                if (isSupportTdscdma()) {
                    mButtonEnabledNetworks.setEntries(
