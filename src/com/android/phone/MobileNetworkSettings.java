...
public class MobileNetworkSettings extends PreferenceActivity
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (subManager.getDefaultDataSubId() != mSubId) {
            // This is confusing!
            if (mButtonPreferredNetworkMode != null) mButtonPreferredNetworkMode.setEnabled(false);
            if (mButtonEnabledNetworks != null) mButtonEnabledNetworks.setEnabled(false);
    ...
    protected void onResume() {
        ...
        if (buttonPreferredNetworkMode != null)  {
            loge("cdesai: buttonPreferredNetworkMode != null");
            mPhone.getPreferredNetworkType(mHandler.obtainMessage(
                    MyHandler.MESSAGE_GET_PREFERRED_NETWORK_TYPE));
        }

        if (buttonEnabledNetworks != null)  {
            loge("cdesai: BUTTON_ENABLED_NETWORKS_KEY != null");
            mPhone.getPreferredNetworkType(mHandler.obtainMessage(
                    MyHandler.MESSAGE_GET_PREFERRED_NETWORK_TYPE));
        }
        ...
        if (subManager.getDefaultDataSubId() != mSubId) {
            ...
            if (buttonPreferredNetworkMode != null) buttonPreferredNetworkMode.setEnabled(false);
            if (buttonEnabledNetworks != null) buttonEnabledNetworks.setEnabled(false);
            // TODO: Set summary like "3G (Current: 2G)" for example on a non multi rat device
            // with this sub set to 3G but the other sub being the active data sub.
        }
    }
    ...
    private void handleRoamingModeChange(int newMode)
    {
        ...
        if (prefNetMode != -1) {
            mPhone.setPreferredNetworkType(prefNetMode,
