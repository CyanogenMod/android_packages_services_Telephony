...
public class MobileNetworkSettings extends PreferenceActivity
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        if (preference.getKey().equals(BUTTON_4G_LTE_KEY)) {
            ...
            int settingsNetworkMode = getConfiguredNetworkSetting();
    ...
    private int getConfiguredNetworkSetting() {
        try {
            return TelephonyManager.getIntAtIndex(mPhone.getContext().getContentResolver(),
                    android.provider.Settings.Global.CONFIGURED_NETWORK_MODE, mPhone.getPhoneId());
        } catch (Settings.SettingNotFoundException e) {
    ...
    private final class NetworkModeObserver extends ContentObserver {
        ...
        private final Uri CONFIGURED_URI =
                Settings.Global.getUriFor(Settings.Global.CONFIGURED_NETWORK_MODE);
        ...
        public void register(boolean register) {
            ...
            if (register) {
                ...
                cr.registerContentObserver(CONFIGURED_URI, false, this);
            } else {
    ...
    private void UpdateEnabledNetworksValueAndSummary(int NetworkMode) {
        int configuredMode = getConfiguredNetworkSetting();
        int networkModeSummaryResId = mEnabledNetworksSummaries.get(NetworkMode, -1);
        int confNetworkModeSummaryResId = mEnabledNetworksSummaries.get(configuredMode, -1);
        if (networkModeSummaryResId == -1) {
            loge("Invalid Network Mode (" + NetworkMode + "). Ignore.");
            mButtonEnabledNetworks.setSummary(null);
        } else {
            mButtonEnabledNetworks.setValue(Integer.toString(configuredMode));
            if (networkModeSummaryResId == confNetworkModeSummaryResId) {
                mButtonEnabledNetworks.setSummary(networkModeSummaryResId);
            } else {
                String summary = getString(R.string.preferred_network_mode_summary_mismatch,
                        getString(confNetworkModeSummaryResId),
